...
#      plugins/healthcare-equity copy.
...
SRC="$SUITE/plugins/healthcare-equity"
REPO="$(cd "$(dirname "$0")/.." && pwd)"

[ -f "$SRC/.claude-plugin/plugin.json" ] || { echo "error: $SRC is not the plugin dir" >&2; exit 1; }

# 1. replace plugin content (items absent in the suite copy are removed here too —
#    healthcare-equity bundles no .mcp.json by design)
for item in skills agents references CLAUDE.md README.md .mcp.json; do
  rm -rf "$REPO/${item:?}"
  [ -e "$SRC/$item" ] && cp -R "$SRC/$item" "$REPO/$item"
done
